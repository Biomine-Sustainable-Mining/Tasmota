...
class lwdecode_cls
  ...
  def lw_decode(data)
    ...
    if !decoder
      log("LwD: decoder attribute not found",3)
      return true
    end

    var payload = device_info['Payload']
    if !payload || payload.size() == 0 
      log("LwD: invalid payload",3)
      return true 
    end
    ...
    if !self.lw_decoders.find(decoder)
      try
        ...
        if LwDeco
          ...
          log(f"LwD: Unable to load {decoder}",3)
          return true
        end
      except .. as e, m
        log(format("Decoder load error: %s", m),2)
        return true
      end
    end
    ...
    if hashCheck
      ...
      if current_hash == self.last_payload_hash 
        log("LwD: Duplicate Hash found, skipping",3)
        return true 
      end
    ...
    try
      decoded = self.lw_decoders[decoder].decodeUplink(
        device_info['Name'],
        device_info['Node'],
        device_info['RSSI'],
        device_info['FPort'],
      ...
      log(f"LwD: Decode error for {device_name}: {e .. ', ' .. m}", 1)
      decoded['Error'] = e .. ', ' .. m
    end
