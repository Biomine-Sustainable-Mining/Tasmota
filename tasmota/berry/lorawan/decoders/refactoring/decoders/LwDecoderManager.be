...
def load_lw_decoder(decoder_name, force_reload)
  ...
  # Check decoder limit
  if global.lw_decoders.size() >= get_lw_config("max_decoders", MAX_DECODERS)
    evict_oldest_decoder()
  end
...
def should_check_hash(decoder_name)
  ...
  return get_lw_config('hash_check_enabled', true)
end
...
def reload_lw_decoder(decoder_name)
  return load_lw_decoder(decoder_name, true)
end
